...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function loadMovies(): Promise<void> {
      try {
        ...
        if (active) {
          setError(requestError instanceof Error ? requestError.message : 'Unable to load movies.');
        }
      } finally {
  ...
  return (
    <main className="discovery-page">
      <section className="discovery-header" aria-labelledby="dashboard-title">
        <p className="eyebrow">RED SEAT / DISCOVER</p>
        <h1 id="dashboard-title">Choose your next screening.</h1>
        <p>Start with a film, then select the theatre that fits your night.</p>
      </section>
